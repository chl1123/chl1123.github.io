...
class _ModbusProtoBase:
    """Modbus 通用协议封装基类，只提供标准功能码读写。"""
    ...
    def execute(self, slave: int, function_code: int, start_addr: int, num: int = 0, output_value=0):
        return self.master.execute(int(slave), int(function_code), int(start_addr), int(num), output_value=output_value)

    # 4x
    def read_holding_registers(self, start_addr: int, num: int, slave: int = 1) -> Tuple[int, ...]:
        """读取保持寄存器"""
        self.check_range("num", int(num), 1, 125)
        return self.master.execute(int(slave), cst.READ_HOLDING_REGISTERS, int(start_addr), int(num))

    def write_single_register(self, addr: int, value: int, slave: int = 1) -> Tuple[int, int]:
        """写单个保持寄存器"""
        return self.master.execute(int(slave), cst.WRITE_SINGLE_REGISTER, int(addr), output_value=int(value))
    ...
    def read_coils(self, start_addr: int, num: int, slave: int = 1) -> Tuple[int, ...]:
        ...
        return self.master.execute(int(slave), cst.READ_COILS, int(start_addr), int(num))

    def write_single_coil(self, addr: int, value: int, slave: int = 1) -> Tuple[int, int]:
        """写单个线圈"""
        coil_value = 1 if int(value) else 0
        return self.master.execute(int(slave), cst.WRITE_SINGLE_COIL, int(addr), output_value=coil_value)
    ...
    def read_input_registers(self, start_addr: int, num: int, slave: int = 1) -> Tuple[int, ...]:
        """读取输入寄存器"""
        self.check_range("num", int(num), 1, 125)
        return self.master.execute(int(slave), cst.READ_INPUT_REGISTERS, int(start_addr), int(num))

    # 1x
    def read_discrete_inputs(self, start_addr: int, num: int, slave: int = 1) -> Tuple[int, ...]:
        """读取离散输入"""
        self.check_range("num", int(num), 1, 2000)
        return self.master.execute(int(slave), cst.READ_DISCRETE_INPUTS, int(start_addr), int(num))


class ModbusRtuProto(_ModbusProtoBase):
    """
...
class ModbusTcpProto(_ModbusProtoBase):
    """
    通用 Modbus-TCP 协议封装（基于 modbus_tk）
    只提供连接、标准功能码读写，不承载业务寄存器语义。
    """

    def __init__(self, host: str = "192.168.192.5", port: int = 502, timeout: float = 3.0):
        self.host = host
        self.port = int(port)
