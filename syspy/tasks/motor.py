...
class MotorControlAction(ActionBase):
    ...
    def _start(self):
        ...
        if self.motor_type != "linear":
            raise ParamError(
                f"motorType '{self.motor_type}' not supported, expected: walk/steer/spin/linear"
            )
    ...
    def _step(self):
        ...
        reached_limit = (
            Motor.isMotorPositionReached(self.key, self._min_length, self._stop_di)
            and self._speed < 0
        ) or (
            Motor.isMotorPositionReached(self.key, self._max_length, self._stop_di)
            and self._speed > 0
        )
