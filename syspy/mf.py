...
class MF(Service):
    ...
    @classmethod
    @call_service()
    def resetGoForkPath(cls, x: float, y: float, yaw: float, back_dist: float, min_ahead_dist: float,
                        ahead_dist: float):
        """重置叉车去往识别点的路径规划
    ...
    @classmethod
    @call_service()
    def setGoodsShapeWithName(cls, head: float, tail: float, width: float, recfile: str):
        """设置货物形状时传入识别文件路径
        Args:
            head (float):
            tail (float):
    ...
    @classmethod
    @call_service()
    def setSafeZone(cls, zoneType: int, maxSpeed: float, autoRestart: bool, muteAudio: bool, muteEnable: bool):
        """
        Args:
            zoneType:
            maxSpeed:
