...
class ConfigParams:
    ...
    def reload_config(cls):
        ...
        # 用户配置的左右开合电机绑定（有两个电机时才生效）
        if len(cls.expand_motor_names) >= 2:
            cfg_left = cfg.get("expandMotorLeft", "")
            cfg_right = cfg.get("expandMotorRight", "")
            if cfg_left:
                cls.expand_motor_left = cfg_left
            if cfg_right:
                cls.expand_motor_right = cfg_right
    ...
    def get_device_model_param(cls):
        ...
        # 读取开合电机（可能是多个，逗号分隔）
        expand_motor_str = RobotParam.getDevice("Model-000", f"moduleType.{cls.module_type}.expandMotor") or ""
        if expand_motor_str:
            cls.expand_motor_names = [m.strip() for m in expand_motor_str.split(',') if m.strip()]
            # 如果有两个开合电机，分别赋值为左右
            if len(cls.expand_motor_names) >= 2:
                cls.expand_motor_left = cls.expand_motor_names[0]
                cls.expand_motor_right = cls.expand_motor_names[1]
    ...
    def get_device_motor_param(cls):
        ...
        if cls.pitch_motor_name:
            cls.pitch_motor_min = float(RobotParam.getDevice(f"{cls.pitch_motor_name}", "basic.minLength") or 0)
            cls.pitch_motor_max = float(RobotParam.getDevice(f"{cls.pitch_motor_name}", "basic.maxLength") or 0)

        # 读取开合电机参数
        if cls.expand_motor_left:
            cls.expand_motor_left_min = float(RobotParam.getDevice(f"{cls.expand_motor_left}", "basic.minLength") or 0)
            cls.expand_motor_left_max = float(RobotParam.getDevice(f"{cls.expand_motor_left}", "basic.maxLength") or 0)
        if cls.expand_motor_right:
            cls.expand_motor_right_min = float(RobotParam.getDevice(f"{cls.expand_motor_right}", "basic.minLength") or 0)
            cls.expand_motor_right_max = float(RobotParam.getDevice(f"{cls.expand_motor_right}", "basic.maxLength") or 0)
    ...
    def _build_module_motor(cls):
        ...
        if cls.pitch_motor_name:
            pitch_motor = {
                ...
                "currentPosition": 0.0,
                "maxLength": cls.pitch_motor_max,
                "minLength": cls.pitch_motor_min
            }
        ...
        if cls.expand_motor_left:
            expand_left_motor = {
                "type": "expand_left",
                "motorKey": cls.expand_motor_left,
                "jogSupport": cls.expandMotorPosition,
                "currentPosition": 0.0,
                "maxLength": cls.expand_motor_left_max,
                "minLength": cls.expand_motor_left_min
            }
            cls.moduleMotor.append(expand_left_motor)

        if cls.expand_motor_right:
            expand_right_motor = {
                "type": "expand_right",
                "motorKey": cls.expand_motor_right,
                "jogSupport": cls.expandMotorPosition,
                "currentPosition": 0.0,
                "maxLength": cls.expand_motor_right_max,
                "minLength": cls.expand_motor_right_min
            }
            cls.moduleMotor.append(expand_right_motor)
                ...
                with builder.CHILDREN():
                    ...
                    if ConfigParams.reach_motor_name:
                        with builder.CHILD(key="reachMotorPosition", name="Reach Motor Position Control",
                            ...
                            builder.TYPE(ParamType.BOOL)
                            builder.DEFAULTVALUE(True)
                    if ConfigParams.expand_motor_names:
                        with builder.CHILD(key="expandMotorPosition", name="Expand Motor Position Control",
                                           desc="开合电机是否位置控制"):
                            builder.TYPE(ParamType.BOOL)
                            builder.DEFAULTVALUE(True)
                        if len(ConfigParams.expand_motor_names) >= 2:
                            with builder.CHILD(key="expandMotorLeft", name="Expand Motor Left",
                                               desc="左开合电机绑定（默认为第一个电机）"):
                                builder.TYPE(ParamType.STRING)
                                builder.DEFAULTVALUE(ConfigParams.expand_motor_names[0])
                            with builder.CHILD(key="expandMotorRight", name="Expand Motor Right",
                                               desc="右开合电机绑定（默认为第二个电机）"):
                                builder.TYPE(ParamType.STRING)
                                builder.DEFAULTVALUE(ConfigParams.expand_motor_names[1])
        ...
        with cls.builder.GROUPS():
            ...
            with cls.builder.GROUP(key="operation", name="Operations", desc="Task script input parameters"):
                ...
                with cls.builder.CHILDREN():
                    ...
                    if ConfigParams.expand_motor_left:
                        with cls.builder.CHILD(key="expand_left", name="Expand Motor Left",
                                               desc="Left expand motor jog or move"):
                            cls.builder.TYPE(ParamType.ARRAY)
                            with cls.builder.CHILDREN():
                    ...
                    if ConfigParams.expand_motor_right:
                        with cls.builder.CHILD(key="expand_right", name="Expand Motor Right",
                                               desc="Right expand motor jog or move"):
                            cls.builder.TYPE(ParamType.ARRAY)
                            with cls.builder.CHILDREN():
                                with cls.builder.CHILD(key="jogStep", name="Jog Step",
...
class Fork(ModuleBase):
    ...
    def run(self, args):
        ...
        if self.opt == "load":
            ...
            self.cage_stack()
        elif self.opt in ["lift", "shift", "pitch", "reach", "expand_left", "expand_right"]:
    ...
    def _build_expand_motor_action(self, rec_action):
        ...
        for hole in rec_action.hole_positions:
            hole_y = hole["holeY"]
            if hole_y < 0 and left_pos is None:
                left_pos = hole_y
            elif hole_y >= 0 and right_pos is None:
                right_pos = hole_y

        # 如果只有一种孔，另一个电机保持当前位置
        if left_pos is None:
            left_pos = Motor.getMotorPos(ConfigParams.expand_motor_left) if ConfigParams.expand_motor_left else 0.0
        if right_pos is None:
            right_pos = Motor.getMotorPos(ConfigParams.expand_motor_right) if ConfigParams.expand_motor_right else 0.0

        Trace.log(
            f"Expand motor target: left={left_pos:.4f}, right={right_pos:.4f}",
            name="fork.task"
        )
    ...
    def load(self):
        ...
        if self.action_id < len(self.action_list):
            ...
            # 取完货后离库位前，抬升货叉就加载货物模型，并触发俯仰电机到最大角度
            if (isinstance(self.action_list[self.action_id], RunMotorByPosition)
                ...
                # 货叉升到位后，控制俯仰电机到最大角度
                if ConfigParams.pitch_motor_name and ConfigParams.pitch_motor_max > 0:
                    self.action_list.insert(
                        self.action_id + 1,
                        RunMotorByPosition(ConfigParams.pitch_motor_name, ConfigParams.pitch_motor_max,
                                           action_name="pitchToMax")
                    )
        ...
        if self.action_id < len(self.action_list):
            ...
            if (isinstance(self.action_list[self.action_id], RunMotorByPosition)
                ...
                if ConfigParams.pitch_motor_name:
                    self.action_list.insert(
                        self.action_id + 1,
                        RunMotorByPosition(ConfigParams.pitch_motor_name, ConfigParams.pitch_motor_min,
                                           action_name="pitchToMin")
                    )
        ...
        if ConfigParams.module_type in ["straddleLiftFork", "counterBalanceFork"]:
            ...
            if fork_height <= ConfigParams.backLaserEnableHeight and not self.set_fork_region_by_height:
                ...
                Navigation.setClearRegion(self.back_laser_clear_region_name, [p["x"] for p in self.fork_points],
                                          ...
                                          [ConfigParams.fork_root_2D_lasers], Coordinate.ROBOT)
                Trace.log(f"set clear region:{self.back_laser_clear_region_name},{self.fork_points}", name="fork.task")
...
class RunExpandMotorsByPosition(BaseAction):
    ...
    def __init__(self, left_motor_name, right_motor_name, left_position, right_position,
    ...
    def run(self):
        if not self.init:
            ...
            self.action_status = ActionStatus.RUNNING
            Trace.log(
                f"RunExpandMotors: left={self.left_motor_name} to {self.left_position}, "
                f"right={self.right_motor_name} to {self.right_position}",
                name="fork.task"
            )
            if self.left_motor_name:
                Motor.setMotorPosition(self.left_motor_name, self.left_position, self.max_speed)
            if self.right_motor_name:
                Motor.setMotorPosition(self.right_motor_name, self.right_position, self.max_speed)

        # 检查两个电机是否都到位
        if self.left_motor_name:
            self.left_reached = Motor.isMotorReached(self.left_motor_name)
        else:
            self.left_reached = True

        if self.right_motor_name:
            self.right_reached = Motor.isMotorReached(self.right_motor_name)
        else:
            self.right_reached = True

        if self.left_reached and self.right_reached:
            self.action_status = ActionStatus.FINISHED
            Trace.log(f"RunExpandMotors finished", name="fork.task")

    def reset(self):
        if self.left_motor_name:
            Motor.resetMotor(self.left_motor_name)
        if self.right_motor_name:
            Motor.resetMotor(self.right_motor_name)
        self.action_status = ActionStatus.RUNNING
        self.init = False
    ...
    def _trace_state(self) -> dict:
        return {
            "action_status": int(self.action_status),
            "left_reached": self.left_reached,
            "right_reached": self.right_reached,
        }
...
class RunMotorByPosition(BaseAction):
    ...
    def run(self):
        ...
        if self.is_reach:
            ...
            Trace.log(f"agv has base shift:{ConfigParams.base_shift}", name="fork.task")
...
class GoPath(BaseAction):
    def __init__(self, args: Optional[dict] = None):
        ...
        """
        Trace.log(f"go path init", name="fork.task")
...
class GoTwoStraightLine(BaseAction):
    def __init__(self, world_target, min_ahead_dist, ahead_dist, back_dist, speed, max_angle=20.0, dec_dist=1.0,
        ...
        # 如果执行原路返回
            ...
            Trace.log(f"points:{points}", name="fork.task")
            if not points:
                self.action_status = ActionStatus.FAILED
