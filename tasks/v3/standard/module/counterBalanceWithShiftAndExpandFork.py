...
# version: 2.0
# Time: 2026/05/27
# description: counterBalanceWithShiftAndExpandFork - 平衡重带横移和开合货叉
# update:
...
def _calc_expand_motor_target(current_pos, hole_offset, min_length, max_length, min_move=0.005):
    """根据识别偏移量计算开合电机目标位置，只做必要位移。"""
    target_pos = current_pos
    if hole_offset is not None:
        target_pos = clamp(hole_offset, min_length, max_length)
        if abs(target_pos - current_pos) < min_move:
            target_pos = current_pos
    return target_pos
        ...
        with cls.builder.GROUPS():
            ...
            with cls.builder.GROUP(key="operation", name="Operations", desc="Task script input parameters"):
                ...
                with cls.builder.CHILDREN():
                    ...
                    if ConfigParams.expand_motor_items:
                        if len(ConfigParams.expand_motor_items) == 2 and all(
                            item.get("side") in ("left", "right") for item in ConfigParams.expand_motor_items
                        ):
                            with cls.builder.CHILD(key="expand_motor_left", name="Expand Left Motor",
                                                   desc="Expand left motor jog or move"):
                                cls.builder.TYPE(ParamType.ARRAY)
                                with cls.builder.CHILDREN():
                                    with cls.builder.CHILD(key="jogStep", name="Jog Step",
                                                           desc="Jog step for left expand motor"):
                                        cls.builder.TYPE(ParamType.FLOAT)
                                        cls.builder.UNIT("m")
                                        cls.builder.SINGLESTEP(0.01)
                                        cls.builder.DEFAULTVALUE(0.1)
                                    with cls.builder.CHILD(key="position", name="Position",
                                                           desc="Target position for left expand motor"):
                                        cls.builder.TYPE(ParamType.FLOAT)
                                        cls.builder.UNIT("m")
                                        cls.builder.SINGLESTEP(0.01)
                                        cls.builder.DEFAULTVALUE(-1)

                            with cls.builder.CHILD(key="expand_motor_right", name="Expand Right Motor",
                                                   desc="Expand right motor jog or move"):
                                cls.builder.TYPE(ParamType.ARRAY)
                                with cls.builder.CHILDREN():
                                    with cls.builder.CHILD(key="jogStep", name="Jog Step",
                                                           desc="Jog step for right expand motor"):
                                        cls.builder.TYPE(ParamType.FLOAT)
                                        cls.builder.UNIT("m")
                                        cls.builder.SINGLESTEP(0.01)
                                        cls.builder.DEFAULTVALUE(0.1)
                                    with cls.builder.CHILD(key="position", name="Position",
                                                           desc="Target position for right expand motor"):
                                        cls.builder.TYPE(ParamType.FLOAT)
                                        cls.builder.UNIT("m")
                                        cls.builder.SINGLESTEP(0.01)
                                        cls.builder.DEFAULTVALUE(-1)
...
class Fork(ModuleBase):
    ...
    def run(self, args):
        ...
        if self.opt == "load":
            ...
            self.cage_stack()
        elif self.opt in ["lift", "shift", "pitch", "reach", "expand", "expand_motor_left", "expand_motor_right"]:
    ...
    def _build_expand_motor_action(self, rec_action):
        """根据识别孔坐标构建开合电机控制 action"""
        if not rec_action.hole_positions:
            Trace.log("No hole positions from rec, skip expand motor", name="fork.task")
            return None

        if len(ConfigParams.expand_motor_names) < 2:
        ...
        for hole in rec_action.hole_positions:
            ...
            if hole_y > 0 and expand_motor_left_offset is None:
                ...
                expand_motor_right_offset = -hole_y

        current_expand_motor_left_pos = (
            Motor.getMotorPos(ConfigParams.expand_motor_left) if ConfigParams.expand_motor_left else 0.0
        )
        current_expand_motor_right_pos = (
            Motor.getMotorPos(ConfigParams.expand_motor_right) if ConfigParams.expand_motor_right else 0.0
        )

        left_motor_info = next(
            (
                motor for motor in ConfigParams.moduleMotor
                if motor["type"] == "expand" and motor.get("side") == "left"
            ),
            None
        )
        right_motor_info = next(
            (
                motor for motor in ConfigParams.moduleMotor
                if motor["type"] == "expand" and motor.get("side") == "right"
            ),
            None
        )

        expand_motor_left_pos = _calc_expand_motor_target(
            current_expand_motor_left_pos,
            expand_motor_left_offset,
            left_motor_info["minLength"] if left_motor_info else current_expand_motor_left_pos,
            left_motor_info["maxLength"] if left_motor_info else current_expand_motor_left_pos,
        )
        expand_motor_right_pos = _calc_expand_motor_target(
            current_expand_motor_right_pos,
            expand_motor_right_offset,
            right_motor_info["minLength"] if right_motor_info else current_expand_motor_right_pos,
            right_motor_info["maxLength"] if right_motor_info else current_expand_motor_right_pos,
        )

        Trace.log(
            f"Expand motor target: "
            f"expand_motor_left current={current_expand_motor_left_pos:.4f}, "
            f"offset={expand_motor_left_offset}, target={expand_motor_left_pos:.4f}; "
            f"expand_motor_right current={current_expand_motor_right_pos:.4f}, "
            f"offset={expand_motor_right_offset}, target={expand_motor_right_pos:.4f}",
            name="fork.task"
        )
        return RunExpandMotorsByPosition(
            ConfigParams.expand_motor_left, ConfigParams.expand_motor_right,
            expand_motor_left_pos, expand_motor_right_pos
        )

    def _build_pitch_down_action(self, use_max_length=False):
        """构建俯仰电机下压动作。"""
        pitch_motor_info = next(
            (motor for motor in ConfigParams.moduleMotor if motor["type"] == "pitch"),
            None
        )
        if not pitch_motor_info:
            Trace.log("No pitch motor found, skip pitch down", name="fork.task")
            return None
        target_position = pitch_motor_info["maxLength"] if use_max_length else pitch_motor_info["minLength"]
        return RunModuleMotorByPosition(
            pitch_motor_info["motorKey"],
            target_position,
            0.05,
            action_name="pitchDown",
        )
    ...
    def load(self):
        ...
        if self.action_id < len(self.action_list):
            ...
            if (isinstance(self.action_list[self.action_id], Rec)
                ...
                if expand_action:
                    self.action_list.append(expand_action)
                ...
                pitch_down_action = self._build_pitch_down_action()
                if pitch_down_action:
                    self.action_list.append(pitch_down_action)
    ...
    def unload(self):
        if not self.operation_init:
            ...
            if not target_pos or target_pos[3] == -1:
                ...
                pitch_down_action = self._build_pitch_down_action(use_max_length=True)
                if pitch_down_action:
                    self.action_list.append(pitch_down_action)
            else:
    ...
    def motor_jog_or_move(self, motor_type):
        ...
        if not self.operation_init:
            ...
            motor_info = None
            expand_side = None
            lookup_motor_type = motor_type
            if motor_type in ["expand_motor_left", "expand_motor_right"]:
                lookup_motor_type = "expand"
                expand_side = "left" if motor_type.endswith("left") else "right"
            for motor in ConfigParams.moduleMotor:
                if motor["type"] != lookup_motor_type:
                    continue
                if expand_side and motor.get("side") != expand_side:
                    continue
                if not expand_side and lookup_motor_type == "expand" and motor.get("side"):
                    continue
                motor_info = motor
                break
            if not motor_info:
...
class RunExpandMotorsByPosition(BaseAction):
    """同时控制左右开合电机到指定位置"""

    def __init__(self, expand_motor_left_name, expand_motor_right_name,
                 expand_motor_left_position, expand_motor_right_position,
                 max_speed=0.1, action_name="RunExpandMotors"):
        super().__init__(action_name)
        self.expand_motor_left_name = expand_motor_left_name
        self.expand_motor_right_name = expand_motor_right_name
        self.expand_motor_left_position = expand_motor_left_position
        self.expand_motor_right_position = expand_motor_right_position
        self.max_speed = max_speed
        self.init = False
        self.expand_motor_left_reached = False
        self.expand_motor_right_reached = False
        self.action_status = ActionStatus.INIT
    ...
    def run(self):
        if not self.init:
            ...
            self.action_status = ActionStatus.RUNNING
            Trace.log(
                f"RunExpandMotors: expand_motor_left={self.expand_motor_left_name} "
                f"to {self.expand_motor_left_position}, "
                f"expand_motor_right={self.expand_motor_right_name} "
                f"to {self.expand_motor_right_position}",
                name="fork.task"
            )
            if self.expand_motor_left_name:
                Motor.setMotorPosition(
                    self.expand_motor_left_name, self.expand_motor_left_position, self.max_speed
                )
            if self.expand_motor_right_name:
                Motor.setMotorPosition(
                    self.expand_motor_right_name, self.expand_motor_right_position, self.max_speed
                )

        # 检查两个电机是否都到位
        if self.expand_motor_left_name:
            self.expand_motor_left_reached = Motor.isMotorReached(self.expand_motor_left_name)
        else:
            self.expand_motor_left_reached = True

        if self.expand_motor_right_name:
            self.expand_motor_right_reached = Motor.isMotorReached(self.expand_motor_right_name)
        else:
            self.expand_motor_right_reached = True

        if self.expand_motor_left_reached and self.expand_motor_right_reached:
            self.action_status = ActionStatus.FINISHED
            Trace.log(f"RunExpandMotors finished", name="fork.task")

    def reset(self):
        if self.expand_motor_left_name:
            Motor.resetMotor(self.expand_motor_left_name)
        if self.expand_motor_right_name:
            Motor.resetMotor(self.expand_motor_right_name)
        self.action_status = ActionStatus.RUNNING
        self.init = False
    ...
    def _trace_state(self) -> dict:
        return {
            "action_status": int(self.action_status),
            "expand_motor_left_reached": self.expand_motor_left_reached,
            "expand_motor_right_reached": self.expand_motor_right_reached,
        }
...
class MoveChassisByY(BaseAction):
    def __init__(self, robot2pos):
        ...
        if self.shiftMotor != "":
            ...
            self.shift = RunModuleMotorByPosition(self.shiftMotor, shift_position + self.y, 0.01)
            chassis_x_args = {
                "x": self.x,
                "y": 0,
                "theta": 0,
