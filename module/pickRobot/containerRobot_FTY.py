...
# @coding: https://seer-group.coding.net/p/order_issue_pool/requirements/issues/6596/detail
import json
import math
import sys
import time
...
class Module(BasicModule):
    def __init__(self, r: SimModule, args):
        ...
        self.check_box_DI = p.loadParam("check_box_DI", type="int", default=10,
                                           comment="检测是否有货的传感器的DI")
                ...
                if self.rec_box_lift:
                    ...
                    if not self.unload_step[7] and self.unload_step[6]:
                        if not self.rec_box_lift_step[2]:
                            if ModuleTool.check_DI(r, self.check_box_DI):
                        ...
                        if not self.rec_box_lift_step[3] and self.rec_box_lift_step[2]:
                            self.rec_box_lift_step[3] = self.lift(r, self.lift_height)
                        if self.rec_box_lift_step[3]:
