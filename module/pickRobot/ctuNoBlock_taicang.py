...
# @Update : 更新抓斗推出料箱流程为：先伸出5cm, 然后抬起手指，然后推出料箱，适用于太仓项目
                ...
                if "mode" in self.state:
                    if self.state.get("mode", 1) == 0:
                        ...
                        if self.h.switch_mode_res['status'] != Hairou.Action.FINISHED:
                            return self.status
                # else:
                #     r.setWarning(f"Hairou firmware needs to be updated")
                #     return self.status
            except Exception as e:
...
class putGoodsS1:
    ...
    def run(self, r, ctu):
        self.status = MoveStatus.RUNNING
        if ctu.stretch_status is not MoveStatus.FINISHED and not self.pre_stretch:
            ctu.stretch(r, 50)
            ctu.checkFingerStatus(r, 0)
        elif ctu.finger_status is not MoveStatus.FINISHED:
            self.pre_stretch = True
            ctu.stretch_status = MoveStatus.RUNNING
            ctu.finger(r, 1)
        elif ctu.stretch_status is not MoveStatus.FINISHED and self.pre_stretch:
            ctu.stretch(r, self.stretchDist)
            # ctu.checkFingerStatus(r, 1)
        else:
            self.status = MoveStatus.FINISHED
