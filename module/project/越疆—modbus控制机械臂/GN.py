...
# @Version: 1.1
# @Project:
# @Coding:https://seer-group.coding.net/p/issue_pool/requirements/issues/3840/detail
# @Update: 增加读取预期值
...
class Module(BasicModule):
    ...
    def run(self, r: SimModule, args):
        ...
        if self.init:
            ...
            self.tpy = args.get("type", None)
            self.modbus_tcp = ModbusTCP(ip=self.ip, port=502, timeout=3)
        ...
        self.report_info['small'] = 0
        self.report_info['NG'] = 0
        self.report_info['GO'] = 0
        if self.tpy =="small":
            if self.res == 1:
                self.report_info['small'] = self.res

        elif self.tpy == "ng" or self.tpy == "go" or self.tpy == "NG" or self.tpy == "GO":
            if self.res == 1:
                self.report_info['GO'] = self.res
            if self.res == 2:
                self.report_info['NG'] = self.res
        else:
            r.setError("请输入类型，small 或者 go 或者 ng")
            self.status = MoveStatus.FAILED
        if self.res != 0:
