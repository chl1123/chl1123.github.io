...
class Module(BasicModule):
    def __init__(self, r: SimModule, args):
        ...
        self.load_rec_diff_height = p.loadParam("load_rec_diff_height", type="float", default=0.12,
                                                comment="取货识别额外高度")
        self.unload_rec_diff_height = p.loadParam("unload_rec_diff_height", type="float", default=0.12,
                                                  comment="放货识别额外高度")
    ...
    def get_handle(self, r: SimModule, args: dict):
        ...
        if not self.get_motor_calib_state(r):
            self.task_list.extend(
                [FingerCan(r, 1), MotorCalib(self.stretch_motor_name), MotorCalib(self.rotate_motor_name),
                 MotorCalib(self.lift_motor_name)])
    ...
    def writeModbus(self, r: SimModule):
        self.task_list.append(
            self.task_list.append(
                ModbusWrite(self.modbus_IP, self.modbus_port, self.writeModbus_value, 0, 6, self.modbus_slaveId)))
        ...
        if self.opt == "load":
            ...
            if self.lift.status is not MoveStatus.FINISHED:
                ...
                if agv.unloadReset and agv.operation == "unload":
                    ...
                    r.setError(
                        f"shelf already has box. DI {self.goods_check_di} is {self.check_di(r, self.goods_check_di)}")
                    self.status = MoveStatus.FAILED
            else:
                self.status = MoveStatus.FINISHED
    ...
    def run(self, r: SimModule, m: Module):
        ...
        if self.init:
            ...
            if self.position is None:
                ...
                self.status = MoveStatus.FAILED
            if self.position < (-m.max_rotate_angle / 180 * math.pi) or self.position > (
                    m.max_rotate_angle / 180 * math.pi):
...
class RecCan3000(TpModule):
    ...
    def run(self, r: SimModule, m: Module):
        ...
        if self.rec_type == "shelf":
            send_angel_res = self.send_recv_can_frame(r, self.send_channel, self.can_id, self.dlc, self.extend,
                                                      self.send_angel_shelf)
            send_x_y_res = self.send_recv_can_frame(r, self.send_channel, self.can_id, self.dlc, self.extend,
                                                    self.send_x_y_shelf)
            send_qr_code1_res = self.send_recv_can_frame(r, self.send_channel, self.can_id, self.dlc, self.extend,
                                                         self.send_qr_code1_shelf)
            send_qr_code2_res = self.send_recv_can_frame(r, self.send_channel, self.can_id, self.dlc, self.extend,
                                                         self.send_qr_code2_shelf)
            send_qr_code3_res = self.send_recv_can_frame(r, self.send_channel, self.can_id, self.dlc, self.extend,
                                                         self.send_qr_code3_shelf)
            send_qr_code4_res = self.send_recv_can_frame(r, self.send_channel, self.can_id, self.dlc, self.extend,
                                                         self.send_qr_code4_shelf)
            ...
            send_x_y_res = self.send_recv_can_frame(r, self.send_channel, self.can_id, self.dlc, self.extend,
                                                    self.send_x_y_box)
            send_qr_code1_res = self.send_recv_can_frame(r, self.send_channel, self.can_id, self.dlc, self.extend,
                                                         self.send_qr_code1_box)
            send_qr_code2_res = self.send_recv_can_frame(r, self.send_channel, self.can_id, self.dlc, self.extend,
                                                         self.send_qr_code2_box)
            send_qr_code3_res = self.send_recv_can_frame(r, self.send_channel, self.can_id, self.dlc, self.extend,
                                                         self.send_qr_code3_box)
            send_qr_code4_res = self.send_recv_can_frame(r, self.send_channel, self.can_id, self.dlc, self.extend,
                                                         self.send_qr_code4_box)
        ...
        if not self.init and self.status != MoveStatus.FINISHED:
            if self.modbus:
                if self.functionCode == 5:
                    ...
                    res = self.modbus.execute(self.slave, cst.WRITE_MULTIPLE_REGISTERS, self.addr,
