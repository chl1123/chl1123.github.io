...
class Module(BasicModule):
    ...
    def log_PLC(self, r:SimModule):
        ...
        r.logDebug(f"[PLC]["
                   ...
                   f"{ge.afterLimitAlarm}|{ge.beforeLimitAlarm}|{ge.liftEncoderDataErr}|"
                   f"{self.PLC.ready}|{self.PLC.stretch_status}|{self.PLC.stretch_position}|{self.PLC.stretch_speed}|"
                   f"{self.PLC.lift_status}|{self.PLC.lift_position}|{self.PLC.lift_speed}|"
    ...
    def man_lift(self, r):
        if ModuleTool.check_DI(r,self.man_up_lift) and not ModuleTool.check_DI(r,self.man_down_lift):
            if self.PLC.ready and not self.man_handel:
                self.man_handel = True
                self.modbus_tcp.execute(1, cst.WRITE_SINGLE_REGISTER, self.lift__height_speed_command, output_value=self.man_lift_speed_up)
