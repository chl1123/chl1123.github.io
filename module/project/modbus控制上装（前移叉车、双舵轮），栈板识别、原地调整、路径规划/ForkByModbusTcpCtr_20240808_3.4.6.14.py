...
class Module(BasicModule):
    def __init__(self, r: SimModule, args):
        ...
        self.rec_file_goods_check = p.loadParam("rec_file_goods_check", type="str",
                                                default="multi/m0002.multi", comment="货物检测识别文件")
    ...
    def run(self, r: SimModule, args: dict):
        ...
        if self.init:
            ...
            if args.get("rec_file", None):
                ...
                self.rec_file_goods_check = args.get("rec_file_goods_check", None)
    ...
    def get_plc_info(self, r):
        c = self.modbus_tcp.execute(1, cst.READ_HOLDING_REGISTERS, self.gantry_sensor_status_addr, 35)
    ...
    def handle_run(self, r: SimModule):
        if self.operation == "forklift":
            ...
            self.rec_pallet(r)
        elif self.operation == "rec_goodsCheck":
            self.rec_goodsCheck(r)
        elif self.operation == "adjust":
            self.adjust(r)
        elif self.operation == "start_changer":
            self.start_changer(r)
        elif self.operation == "stop_changer":
            self.stop_changer(r)
        else:
    ...
    def rec_goodsCheck(self, r):
        if self.init_operation:
            self.task_list.append(goodsCheck(self.rec_file_goods_check))
            self.init_operation = False
        else:
            self.handle(r)
...
class Rec:
    ...
    def run(self, r: SimModule, m: Module):
        ...
        if rec_status == 3 or rec_status == -1:  # 识别失败的状态
            ...
            self.result: dict = r.getRecResult(1)
...
class goodsCheck:
    ...
    def run(self, r: SimModule, m: Module):
        ...
        task_state = dict()
        rec_status = r.getRecStatus()  # 获取识别状态 0: 初始化, 1: 识别中, 2: 获得结果, 3：识别出错, -1: 未知错误
        if rec_status == 2 or rec_status == 3 or rec_status == -1:  # 识别成功或失败
            # r.setNotice("rec failed:{}".format(self.result))
            if ModuleTool.delay(0.2):
                self.rec_times = self.rec_times + 1
                if r.warningExits(54906) and r.warningExits(54901):
                    self.exceed_times = self.exceed_times + 1
                elif r.warningExits(54905) and r.warningExits(54901):
                    self.empty_times = self.empty_times + 1
                if self.exceed_times > self.max_exceed_times:
                    r.setUserError(53930, f"栈板货物超限")
                    r.clearNotice(57300)
                    self.status = MoveStatus.FAILED
                    self.goodsError = True
                    return
                if self.empty_times > self.max_empty_times:
            ...
            # r.clearWarning(54905)
            # r.clearWarning(54906)
            r.doRecWithAngle(self.filename, 0.0)

        task_state['exceed_times'] = self.exceed_times
        task_state['empty_times'] = self.empty_times
        task_state['rec_count'] = self.rec_times
        task_state['goodsCheck_task_status'] = self.status
        task_state['rec_status'] = rec_status
        task_state['file'] = self.filename
        task_state['goodsError'] = self.goodsError
        m.report_data['goodsCheck_info'] = task_state
        task_state["status"] = self.status
        r.logDebug(json.dumps(task_state))
